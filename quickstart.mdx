...
Auto-dub an audio or video file into another language while preserving the original voice.

<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST https://apis.finevoice.ai/v1/audio/dubbinggen \
      -H "Authorization: Bearer $FINEVOICE_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "sourceUrl": "https://example.com/english_video.mp4",
        "sourceType": "video",
  ...
  <Tab title="Python">
    ...
    res = requests.post(f"{BASE_URL}/v1/audio/dubbinggen", headers=HEADERS,
                        json={"sourceUrl": "https://example.com/english_video.mp4",
                              "sourceType": "video",
                              "sourceLanguageCode": "en",
                              "targetLanguageCode": "zh",
                              "speakers": ["leo"],
